...
def main():
    ...
    print(f"Statements directory: {STATEMENT_DIRECTORY}")
    """
    ...
    for filename in files:
        ...
        if stmt.skipped:  # if the statement is skipped, don't bother testing it
            ...
            test_results = consistency_checks(stmt)
            if test_results:
                # statements.append(stmt)
                print(f"Statement {filename} for account: {stmt.account_number} passed all tests.")
                prepare_export_data(stmt)
            else:
                raise Exception(f"Statement {stmt.id} failed tests. This statement dated {stmt.statement_date_desc} is invalid.")
    ...
    df.to_csv(f"{LOG_DIRECTORY}/log_csv_{current_time}.csv", index=False)
    df.to_excel(  # type: ignore
        excel_writer=f"{LOG_DIRECTORY}/log_excel_{current_time}.xlsx",
        index=False,
    )
    print(
        ...
        f"The number of files processed should match the number of files in the statements directory."
        f"A file may be skipped if it doesn't contain any transactions."
    )
    print_splitter()


def consistency_checks(statement: Statement) -> bool:
    """
    ...
    if round(movement_statement, 2) == round(movement_transaction_blocks, 2) == round(movement_days, 2) == round(movement_transactions, 2):
        print("SUCCESS! Statement balance checks are all GOOD")
        return True
    else:
        print("FAILURE! Statement balance checks do not all match - please check the statement and re-try")
        return False
